...
notebook.add(tab0, text='General')
notebook.add(tab1, text='Speed')
notebook.add(tab2, text='Extension')
notebook.add(tab3, text='Load')
...
#Dispaly in a Label
logoLabel = tk.Label(tabs[GENERAL_TAB], image=photo)
logoLabel.grid(row=1, rowspan=2, column=1, columnspan=3, padx=(110, 40), pady=(12, 0), sticky="w")

tk.Label(tabs[GENERAL_TAB], text="Select COM Port:").grid(row=0, column=0, columnspan=2, padx=20, pady=10, sticky="w")
com_port_var = tk.StringVar()
com_ports = list_serial_ports()
com_port_menu = ttk.Combobox(tabs[GENERAL_TAB], textvariable=com_port_var, values=com_ports, state="readonly")
com_port_menu.grid(row=0, column=0, columnspan=2,  padx=(150, 0), pady=10, sticky="w")
if com_ports:
    com_port_var.set(com_ports[0])

tk.Label(tabs[GENERAL_TAB], text="Baud Rate:").grid(row=0, column=2, padx=(30, 0), pady=10, sticky="w")
baud_rate_var = tk.StringVar(value="115200")
baud_rates = ["9600", "19200", "38400", "57600", "115200", "230400", "460800", "921600"]
baud_menu = ttk.Combobox(tabs[GENERAL_TAB], textvariable=baud_rate_var, values=baud_rates, state="readonly")
baud_menu.grid(row=0, column=3, padx=10, pady=10, sticky="w")

tk.Label(tabs[GENERAL_TAB], text="Timeout (s):").grid(row=0, column=4, padx=(30, 0), pady=10, sticky="w")
timeout_var = tk.StringVar(value="0.2")
timeout_entry = ttk.Entry(tabs[GENERAL_TAB], textvariable=timeout_var, width=5)
timeout_entry.grid(row=0, column=5, columnspan=2, padx=0, pady=10, sticky="w")

auto_scroll_var = tk.BooleanVar(value=True)
auto_scroll_check = ttk.Checkbutton(tabs[GENERAL_TAB], text="Auto-scroll", variable=auto_scroll_var)
auto_scroll_check.grid(row=0, column=6, rowspan=2, padx=(60, 10), pady=(0, 40), sticky="w")

# Group 1: Bridge Control
system_frame = ttk.LabelFrame(tabs[GENERAL_TAB], text="Bridge Control", padding=(10, 5))
...
# Group 2: Direction
direction_frame = ttk.LabelFrame(tabs[SPEED_TAB], text="Direction", padding=(10, 5))
...
position_frame = ttk.LabelFrame(tabs[EXTENSION_TAB], text="Drive To Distance", padding=(10, 5))
...
# Group 4: Homing
position_frame = ttk.LabelFrame(tabs[SPEED_TAB], text="Homing", padding=(10, 5))
...
execution_frame = ttk.LabelFrame(tabs[SPEED_TAB], text="Execution Control", padding=(10, 5))
...
rstPI_frame = tk.Frame(tabs[GENERAL_TAB])
...
control_frame = tk.Frame(tabs[GENERAL_TAB])
...
border_frame = tk.LabelFrame(tabs[GENERAL_TAB], text="Response", background="#D4E2EE", padx=2, pady=2) 
...
for i in range(8):
    ...
    label_entry = tk.Label(tabs[GENERAL_TAB], textvariable=label_var, width=20, anchor='w') if (i > 1) else \
                  tk.Label(tabs[SPEED_TAB], textvariable=label_var, width=20, anchor='w')
    label_vars.append(label_var)

    entry = ttk.Entry(tabs[GENERAL_TAB], width=15) if (i > 1) else ttk.Entry(tabs[SPEED_TAB], width=15)
    float_entries.append(entry)

    sendButton = ttk.Button(tabs[GENERAL_TAB], text="Send", command=lambda idx=i: send_hex_cmd_frame(SEND, idx)) if (i > 1) else \
                 ttk.Button(tabs[SPEED_TAB], text="Send", command=lambda idx=i: send_hex_cmd_frame(SEND, idx))
    send_btn.append(sendButton)
    getButton = ttk.Button(tabs[GENERAL_TAB], text="Get", command=lambda idx=i: send_hex_cmd_frame(GET, idx)) if (i > 1) else \
                ttk.Button(tabs[SPEED_TAB], text="Get", command=lambda idx=i: send_hex_cmd_frame(GET, idx))
